    ...
    def new_tetromino(self):
        if not self.tetromino == None:
            ...
            for background in colliding:
                tetromino = Tetromino(
                    x=background.rect.x, y=background.rect.y, name=self.tetromino.name + "_color")
                self.all_sprites.add(tetromino)
                self.all_obstacles.add(tetromino)
                self.tetrominoes.add(tetromino)
            self.tetromino.kill()
            full_rows = list(range(1, 21))
            for background in self.backgrounds:
                if not pygame.sprite.spritecollide(background, self.tetrominoes, False, pygame.sprite.collide_mask):
